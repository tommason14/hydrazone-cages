...
parser.add_argument(
    ...
    "--cycle-max",
    help=
    "Maximum temperature reached during annealing cycles, in kelvin. Default = 300 K",
    type=float,
    default=300,
)
parser.add_argument(
    ...
    "--cycle-min",
    help=
    "Minimum temperature reached during annealing cycles, in kelvin. Default = 10 K",
    type=float,
    default=10,
)
parser.add_argument(
...
def gen_simulation(
    ...
    # VV NH thermostat doesn't support temperature reassignment so use built-in Langevin
    if is_drude:
        print("Drude Langevin thermostat: 5.0 /ps, 20 /ps")
        integrator = mm.DrudeLangevinIntegrator(T * kelvin, 5.0 / ps,
                                                1 * kelvin, 20 / ps, dt * ps)
...
if __name__ == "__main__":
    ...
    with open("final_structure.gro", "w") as f:
        oh.GroFile.writeFile(simulation.topology,
                             positions,
                             vectors,
                             f,
                             time=time,
